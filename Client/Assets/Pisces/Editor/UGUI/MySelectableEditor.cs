/****************
 *@class name:		MySelectableEditor
 *@description:		
 *@author:			selik0
 *@date:			2023-02-02 14:10:47
 *@version: 		V1.0.0
*************************************************************************/
...
{
    [CustomEditor(typeof(MySelectable), true)]
    /// <summary>
    /// Custom Editor for the MySelectable Component.
    /// Extend this class to write a custom editor for a component derived from MySelectable.
    /// </summary>
    public class MySelectableEditor : Editor
    {
        ...
        private static List<MySelectableEditor> s_Editors = new List<MySelectableEditor>();
        private static bool s_ShowNavigation = false;
        private static string s_ShowNavigationKey = "SelectableEditor.ShowNavigation";

        // Whenever adding new SerializedProperties to the MySelectable and SelectableEditor
        // Also update this guy in OnEnable. This makes the inherited classes from MySelectable not require a CustomEditor.
        ...
        protected virtual void OnEnable()
        {
            ...
            m_PropertyPathToExcludeForChildClasses = new[]
            {
                ...
                m_GraphicsProperty.propertyPath,
            };

            var trans = GetTransition(m_TransitionProperty);
            m_ShowColorTint.value = (trans == MySelectable.Transition.ColorTint);
            m_ShowSpriteTrasition.value = (trans == MySelectable.Transition.SpriteSwap);
            m_ShowAnimTransition.value = (trans == MySelectable.Transition.Animation);
        ...
        static MySelectable.Transition GetTransition(SerializedProperty transition)
        {
            return (MySelectable.Transition)transition.enumValueIndex;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (graphic == null)
                graphic = (target as MySelectable).GetComponent<Graphic>();

            var animator = (target as MySelectable).GetComponent<Animator>();
            m_ShowColorTint.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == MyButton.Transition.ColorTint);
            m_ShowSpriteTrasition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == MyButton.Transition.SpriteSwap);
            m_ShowAnimTransition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == MyButton.Transition.Animation);

            EditorGUILayout.PropertyField(m_TransitionProperty);

            ++EditorGUI.indentLevel;
            {
                if (trans == MySelectable.Transition.ColorTint || trans == MySelectable.Transition.SpriteSwap)
                {
                ...
                if (EditorGUILayout.BeginFadeGroup(m_ShowAnimTransition.faded))
                {
                    ...
                    if (animator == null || animator.runtimeAnimatorController == null)
                    {
                        ...
                        {
                            var controller = GenerateSelectableAnimatorContoller((target as MySelectable).animationTriggers, target as MySelectable);
                            if (controller != null)
                            {
                                if (animator == null)
                                    animator = (target as MySelectable).gameObject.AddComponent<Animator>();
            ...
            if (m_GraphicsProperty.arraySize <= 0)
            {
                MySelectable selectable = serializedObject.targetObject as MySelectable;
                List<Graphic> list = new List<Graphic>();
                selectable.ResetGraphics();
            }
            EditorGUILayout.PropertyField(m_GraphicsProperty);

            // We do this here to avoid requiring the user to also write a Editor for their MySelectable-derived classes.
        ...
        // We need to make sure that m_PropertyPathToExcludeForChildClasses has all the MySelectable properties and in the correct order.
        ...
        private static Animations.AnimatorController GenerateSelectableAnimatorContoller(AnimationTriggers animationTriggers, MySelectable target)
        {
            if (target == null)
                return null;
        ...
        private static string GetSaveControllerPath(MySelectable target)
        {
        ...
        private static string BuildAnimationPath(MySelectable target)
        {
        ...
        private static void StaticOnSceneGUI(SceneView view)
        {
            ...
            MySelectable[] selectables = MySelectable.allSelectablesArray;

            for (int i = 0; i < selectables.Length; i++)
            {
                MySelectable s = selectables[i];
                if (SceneManagement.StageUtility.IsGameObjectRenderedByCamera(s.gameObject, Camera.current))
                    DrawNavigationForSelectable(s);
            }
        }

        private static void DrawNavigationForSelectable(MySelectable sel)
        {
            if (sel == null)
                return;
        ...
        private static void DrawNavigationArrow(Vector2 direction, MySelectable fromObj, MySelectable toObj)
        {
